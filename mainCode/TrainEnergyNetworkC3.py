...
# -*- coding: utf-8 -*-
"""
Created on Tue Apr 21 15:58:54 2020
...
from tensorflow.keras.layers import Conv3D, MaxPooling3D, AveragePooling3D, Dense, Flatten,Dropout, Input, Activation, BatchNormalization
...
#            for y1 in range(y):
#                for z1 in range(z):
#                    Array[i,x1,y1,z1]=np.log(Array[i,x1,y1,z1]+1)
#        end3=time.time()
#        sum3+=end3-start3
#    
#    print("sum3=",sum3)
...
#    print(end1-start1)
    ...
    xtrain,xtest=np.split(Array,[int(nevents-nevents/8)])
    ytrain,ytest=np.split(Classifier,[int(nevents-nevents/8)])
    print(xtrain.shape)
    print(xtest.shape)
    print(ytest.shape)
    print(ytrain.shape)
...
#    model.add(BatchNormalization())
    model.add(Conv3D(20,kernel_size=(8,8,8),padding='same',input_shape=input_shape1))
#    model.add(BatchNormalization())
    model.add(Activation('relu'))
    model.add(MaxPooling3D(pool_size=(2,2,2)))
    model.add(Conv3D(10,kernel_size=(4,4,4),padding='same'))
#    #model.add(BatchNormalization())
    model.add(Activation('relu'))
    model.add(MaxPooling3D(pool_size=(2,2,2)))
...
#    model.add(Dropout(D))
    ...
    estimator=model.fit(xtrain,ytrain,validation_data=(xtest,ytest)
                        ,epochs=20,batch_size=256,verbose=1,callbacks=[mc])
